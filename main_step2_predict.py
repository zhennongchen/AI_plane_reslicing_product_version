...
task_list = ['s_lv','seg','2C-r','2C-t','3C-r','3C-t','4C-r','4C-t','BASAL-r','BASAL-t'] 
task_num_list = list(range(1,10)) # select the task 

###########
# define patient CT image list
img_list = ff.find_all_target_files(['*/img-nii-1.5/*.nii.gz'],cg.main_dir)
...
for task_num in task_num_list:
  ...
  ##### load saved weight
  model_file = os.path.join(cg.model_dir,'model-U_allbatches_'+task_list[task_num]+'.hdf5')
  ...
  for img in img_list:
    ...
    if task_num == 0:
      ...
      save_dir = os.path.join(cg.main_dir,patient_id,'seg-pred-lv')
      os.makedirs(save_dir,exist_ok = True)
      save_path = os.path.join(save_dir,'seg_lv_'+os.path.basename(img))
      nb.save(u_pred, save_path)

    if task_num == 1:
      # check whether already done:
      if os.path.exists(os.path.join(cg.main_dir,patient_id,'seg-pred','seg_'+str(time_frame)+'.nii.gz')):
        print('Already done segmentation. SKIPPING')
        continue
      u_gt_nii = nb.load(img)
      u_pred = np.argmax(u_pred[0], axis = -1).astype(np.uint8)
      u_pred = dv.crop_or_pad(u_pred, u_gt_nii.get_data().shape)
      u_pred = nb.Nifti1Image(u_pred, u_gt_nii.affine)
      save_dir = os.path.join(cg.main_dir,patient_id,'seg-pred')
      os.makedirs(save_dir,exist_ok = True)
  ...
  # save vectors
    if task_num > 1:
      if os.path.exists(os.path.join(cg.main_dir,patient_id,'vectors-pred',task_list[task_num]+'.npy')):
        print('Already done vector. SKIPPING')
        continue
      x_n = ff.normalize(x_pred)
      y_n = ff.normalize(y_pred)
      matrix = np.concatenate((t_pred.reshape(1,3),x_n.reshape(1,3),y_n.reshape(1,3)))
      save_dir=os.path.join(cg.main_dir,patient_id,'vectors-pred')
      os.makedirs(save_dir, exist_ok = True)
      if time_frame == 0:
